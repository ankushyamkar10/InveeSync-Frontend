...
import {
  useFetchProcesses,
  useFetchProcessSteps,
  useCreateProcessStepsMutation,
...
const ProcessSteps = () => {
  ...
  const validateForm = () => {
    ...
    if (
      !conversionRatio ||
      isNaN(conversionRatio) ||
      parseFloat(conversionRatio) < 0
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await createProcessStepsMutation.mutateAsync({
        formData,
      });
    ...
    <div className="max-w-4xl mx-auto bg-white">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-6">
          Define manufacturing sequence and parameters
        </p>
      ...
      {selectedItem &&
        ...
        (processSteps && processSteps.length > 0 ? (
                ...
                {isLoadingSteps ? (
                  ...
                  <tr className="text-gray-900">
                    <td colSpan="6" className="text-center text-red-500">
                      Error loading process steps. Please try again.
                    </td>
                  </tr>
                ) : (
                  processSteps?.map((processStep, index) => {
                    const itemName =
                      itemLookup[processStep.item_id] || "Unknown Item";
                    const { name: processName, type: processType } =
                      processLookup[processStep.process_id] || {
          ...
          <div className="py-6 border border-gray-200 text-center text-gray-900">
            No Data for above selected pair
          </div>
        ))}
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-control w-full max-w-md mb-4">
            ...
            <input
              ...
              className="input input-borderd w-full bg-white text-gray-900 disabled:bg-white disabled:border-gray-200 disabled:placeholder:text-gray-900"
          ...
          <button
            ...
            disabled={
              ...
              !sequence ||
              !conversionRatio ||
              createProcessStepsMutation.isLoading
            }
          >
            {createProcessStepsMutation.isLoading
              ? "Adding..."
